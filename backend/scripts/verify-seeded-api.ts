...
if (!token) throw new Error('Set VERIFY_API_TOKEN to a token returned by POST /auth/login.');
...
try {
  ...
  for (let page = 1; page <= first.body.meta.totalPages; page++) {
    const { body } = await call(
      'list page',
      'GET',
      `/employees?status=ALL&pageSize=100&page=${page}`,
    );
    for (const e of body.data) {
      seen.add(e.id);
      const key = `${e.fullName}\u0000${e.id}`;
      if (previousKey && key < previousKey && e.fullName === previousKey.split('\u0000')[0])
        ordered = false;
      previousKey = key;
    }
  }
  check(
    'walking every page yields every employee exactly once',
    seen.size === total,
    `${seen.size}/${total}`,
  );
  ...
  const beyond = await call(
    'page beyond end',
    'GET',
    '/employees?status=ALL&pageSize=100&page=101',
  );
  check(
    'page past the end: 200, empty, accurate totals',
    beyond.status === 200 && beyond.body.data.length === 0 && beyond.body.meta.totalItems === total,
  );
  ...
  const terminatedList = await call(
    'terminated list',
    'GET',
    '/employees?status=TERMINATED&pageSize=1',
  );
  ...
  check(
    'filter-options lists countries, departments and job titles',
    options.body.data.countries.length > 0 &&
      options.body.data.departments.length === 10 &&
      options.body.data.jobTitles.length > 0,
  );
  ...
  for (const department of options.body.data.departments as string[]) {
    const api = await call(
      'filter department',
      'GET',
      `/employees?status=ALL&pageSize=1&department=${department}`,
    );
    const db = await prisma.employee.count({ where: { department: department as never } });
    check(
      `department=${department}`,
      api.body.meta.totalItems === db,
      `api=${api.body.meta.totalItems} db=${db}`,
    );
  }
  for (const { code } of options.body.data.countries as { code: string }[]) {
    const api = await call(
      'filter country',
      'GET',
      `/employees?status=ALL&pageSize=1&country=${code}`,
    );
  ...
  for (const title of (options.body.data.jobTitles as string[]).slice(0, 5)) {
    const api = await call(
      'filter jobTitle',
      'GET',
      `/employees?status=ALL&pageSize=1&jobTitle=${encodeURIComponent(title)}`,
    );
  ...
  const combo = await call(
    'filter combo',
    'GET',
    '/employees?department=ENGINEERING&country=DE,US&pageSize=1',
  );
  ...
  check(
    'department + multiple countries + default status',
    combo.body.meta.totalItems === comboDb,
    `api=${combo.body.meta.totalItems} db=${comboDb}`,
  );

  // -- search ---------------------------------------------------------------------
  const sample = await prisma.employee.findFirstOrThrow({
  ...
  const byEmail = await call(
    'search email',
    'GET',
    `/employees?q=${encodeURIComponent(sample.email)}`,
  );
  check(
    'search by exact email finds the employee',
    byEmail.body.data.some((e: { id: string }) => e.id === sample.id),
  );
  const byNumber = await call(
    'search number',
    'GET',
    `/employees?q=${encodeURIComponent(sample.employeeNumber.toLowerCase())}&status=ALL`,
  );
  check(
    'search by employee number (case-insensitive)',
    byNumber.body.data.some((e: { id: string }) => e.id === sample.id),
  );
  ...
  check(
    'search by name, any word order, any case',
    byName.body.data.some((e: { id: string }) => e.id === sample.id),
    `q="${nameWords}"`,
  );
  const dbNameMatches = await prisma.employee.count({
    where: {
      ...
      AND: nameWords
        .split(' ')
        .map((w) => ({ fullName: { contains: w, mode: 'insensitive' as const } })),
    },
  });
  check(
    'name-search total matches an independent count (name column only)',
    byName.body.meta.totalItems >= dbNameMatches,
  );
  const nothing = await call('search none', 'GET', '/employees?q=zzzzqqqqxxxx');
  check(
    'search with no match: 200, empty, totalItems 0',
    nothing.status === 200 && nothing.body.meta.totalItems === 0,
  );

  // -- retrieval, incl. current salary vs the database ------------------------------
  const sampleIds = await prisma.employee.findMany({
    take: 50,
    skip: 1000,
    orderBy: { id: 'asc' },
    select: { id: true },
  ...
  for (const { id } of sampleIds) {
    ...
    const dbSalary = await prisma.salaryRecord.findFirst({
    ...
    if (
      ...
      !dbSalary ||
      !apiSalary ||
      apiSalary.amount !== dbSalary.amount.toFixed(2) ||
      apiSalary.currencyCode !== dbSalary.currencyCode
    )
      salaryMismatches++;
  }
  check(
    '50 sampled employees: detail salary equals the DB current salary, exactly',
    salaryMismatches === 0,
    `${salaryMismatches} mismatches`,
  );
  ...
  check(
    'pageSize=1000 -> 400',
    (await call('bad size', 'GET', '/employees?pageSize=1000')).status === 400,
  );
  check(
    'unknown filter value -> 400',
    (await call('bad dept', 'GET', '/employees?department=WIZARDS')).status === 400,
  );
  check('malformed id -> 400', (await call('bad id', 'GET', '/employees/xyz')).status === 400);
  check(
    'unknown id -> 404',
    (await call('missing', 'GET', '/employees/00000000-0000-4000-8000-000000000000')).status ===
      404,
  );
  ...
  try {
    ...
    check(
      'edit -> 200',
      (await call('patch', 'PATCH', `/employees/${id}`, { jobTitle: 'Senior Verifier' })).status ===
        200,
    );
    const deactivated = await call('delete', 'DELETE', `/employees/${id}`);
    check(
      'delete -> 200 and TERMINATED',
      deactivated.status === 200 && deactivated.body.data.employmentStatus === 'TERMINATED',
    );
  } finally {
  ...
  check(
    `no request slower than ${SLOW_MS} ms`,
    slow.length === 0,
